...
impl Syscall<'_> {
    pub async fn sys_futex_wait(
        ...
        let futex = proc.get_futex(value_ptr);
    ...
    pub fn sys_futex_requeue(
        ...
        let wake_futex = proc.get_futex(value_ptr.clone());
        let requeue_futex = proc.get_futex(value_ptr);
        wake_futex.requeue(
            current_value,
            wake_count as usize,
    ...
    pub fn sys_futex_wake_single_owner(&self, value_ptr: UserInPtr<AtomicI32>) -> ZxResult {
        ...
        let proc = self.thread.proc();
        proc.get_futex(value_ptr).wake_single_owner();
        Ok(())
    }
}

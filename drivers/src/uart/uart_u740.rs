    ...
    T::Value: From<u8> + TryInto<u8> + From<u32> + TryInto<u32>,
{
    fn init(&mut self) {
        ...
        self.tx_ctrl.write((
            (self.tx_ctrl.read().try_into().unwrap_or(0) | TXCTRLFlags::TXEN.bits() )
        ).into());

        // Enable receive and set interrupt watermark
        self.rx_ctrl.write((
            (self.rx_ctrl.read().try_into().unwrap_or(0) | RXCTRLFlags::RXEN.bits())
        ).into());
    ...
    fn try_recv(&mut self) -> DeviceResult<Option<u8>> {
        ...
        if RXDATAFlags::from_bits_truncate(
            ch.try_into().unwrap_or(0)
    ...
    fn send(&mut self, ch: u8) -> DeviceResult {
        ...
        loop {
            ...
            if !TXDATAFlags::from_bits_truncate(
                status.try_into().unwrap_or(0)

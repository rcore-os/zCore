...
use crate::error::{SysResult, LxError as SysError};
use spin::Mutex;
...
impl Socket for TcpSocketState {
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
        ...
        */
        error!("Tcp read() {} unimplemented !", data[0]);
        return (
            Err(SysError::ENOTCONN),
            Endpoint::Ip(IpEndpoint::UNSPECIFIED),
            );
                    ...
                    loop {
                        ...
                        match socket.state() {
                            TcpState::SynSent => {
                                ...
                                error!("Tcp connect() SOCKET_ACTIVITY unimplemented !");
                            }
    ...
    fn accept(&mut self) -> Result<(Box<dyn Socket>, Endpoint), SysError> {
        ...
        loop {
            ...
            drop(socket);
            //SOCKET_ACTIVITY.wait(sockets);
            error!("Tcp accept() SOCKET_ACTIVITY unimplemented !");
        }
    }
...
impl Socket for UdpSocketState {
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
        loop {
            ...
            drop(socket);
            //SOCKET_ACTIVITY.wait(sockets);
            error!("Udp read() SOCKET_ACTIVITY unimplemented !");
        }
    }
        ...
        match request {
            ...
            0x8954 => {
                ...
                if let AddressFamily::Internet = AddressFamily::from(req.arp_pa.family) {
                    ...
                    let ifname = unsafe {
                        str::from_utf8(slice::from_raw_parts(name, req.arp_dev.len())).unwrap()
                    };
...
impl Socket for RawSocketState {
    fn read(&self, data: &mut [u8]) -> (SysResult, Endpoint) {
        loop {
            ...
            drop(socket);
            //SOCKET_ACTIVITY.wait(sockets);
            error!("RawSocket read() SOCKET_ACTIVITY unimplemented !");
        }
    }
...
enum_with_unknown! {
    ...
    pub enum NetlinkMessageType(u16) {
        /// Nothing
        Noop = 1,
        /// Error
        Error = 2,

...
impl Syscall<'_> {
    ...
    pub async fn syscall(&mut self, num: u32, args: [usize; 6]) -> isize {
        ...
        let raw_elapsed_ns = kernel_hal::timer::timer_now()
            ...
            .unwrap_or_default()
            .as_nanos() as u64;
        // Clamp sleeping/awaiting time so idle processes in poll/epoll/futex don't inflate stime.
        let elapsed_ns = if raw_elapsed_ns > 1_000_000 { 100_000 } else { raw_elapsed_ns };
